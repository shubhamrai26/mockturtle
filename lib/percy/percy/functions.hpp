...
template<typename Fn>
std::array<syn_result,10> synthesize_func_general_primitive( kitty::dynamic_truth_table const& tt, Fn&& fn, mockturtle::xmg_network& xmg )
{
  percy::chain chain;
  percy::spec spec;
  spec.fanin = 3;
  ...
  percy::printer<std::string> pp( chain );
  percy::npn_db_populate<std::string> npn_db( chain, xmg );
  percy::bsat_wrapper solver;
  percy::ssv_encoder encoder(solver);
  add_all_three_input_primitives( spec, fn, pp );

  spec[0] = tt;
  std::cout << "printing binary" ; 
  ...
  //if(n == 0) 
  //{
  //    std::cout << "nly synthesize " << std::endl;
  ...
  //    return syn_result{chain.get_nr_steps(), pp(), npn_db(), dur};
  //}
  std::array<syn_result, 10> result;
  for (uint32_t i = 0; i  <10 ; i++)
  {
      auto n_beg = std::chrono::high_resolution_clock::now();
      auto n_result = percy::next_struct_solution( spec, chain, solver, encoder); 
      auto n_end = std::chrono::high_resolution_clock::now();
      auto n_dur = std::chrono::duration_cast<std::chrono::microseconds>( n_end - n_beg ).count();
      
      assert ( n_result == percy::success );
      assert( chain.simulate()[0] == spec[0] );

      result[i] = syn_result{chain.get_nr_steps(), pp(), npn_db(), n_dur};
  }
  return result;
}
...
void synthesize_all_npn_classes( Fn&& fn, int num_inputs, std::string ofname = "logfile" )
{
  ...
  for ( int i = 0; i < all_classes.size(); i++ )
  {
    ...
    std::array<syn_result,10> sr;
    std::cout << "iterations #" << i <<  std::endl;
    sr = synthesize_func_general_primitive( dtt, fn, xmg);

    for (int i =0; i < 10; i++)
    {
        nc[sr[i].n] += 1;
        nf[sr[i].n] += func_count[cls];
        std::cout << "\tClass " << cls << " needs " << sr[i].n << " gates" << std::endl;
        std::cout << "\tClass " << cls << " chain: " << sr[i].chain << std::endl;

        outf.open( ofname, std::ios_base::app );
        outf << "0x" << cls << "," << func_count[cls] << "," << sr[i].n << "," << sr[i].chain << "," << sr[i].syn_time << std::endl;
        outf.close();
    }
  }
  ...
  tt_outf.open( db_file_name, std::ios_base::app );
  ...
  xmg.foreach_node( [&]( auto const& node ) {
    xmg.foreach_fanin( node, [&]( auto const& fi, auto index ) {
      ...
      //std::cout << "Fanins" << xmg.is_complemented(fi) << " " << xmg.get_node(fi) << " " <<std::endl; 
      if ( index == 0u )
      {
        encoded_number <<= 1;
